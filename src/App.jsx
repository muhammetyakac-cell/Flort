...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('flort_user');
  ...
  useEffect(() => {
    ...
    if (currentUser.is_admin) fetchIncomingThreads();
  }, [currentUser]);

  useEffect(() => {
    if (!selectedProfileId || !currentUser || currentUser.is_admin) return;
    fetchMessages(selectedProfileId);
  }, [selectedProfileId, currentUser]);

  async function signUp() {
    if (mode === 'admin') {
      setStatus('Admin kayıt olamaz.');
      return;
    }
  ...
  async function signIn() {
    ...
    if (mode === 'admin') {
      ...
      if (fetchError) {
        setLoading(false);
        return setStatus(fetchError.message);
      }
      ...
      if (!adminRow) {
        ...
        if (createError) {
          setLoading(false);
          return setStatus(createError.message);
        }
      }

      const adminUser = { username: ADMIN_USERNAME, is_admin: true };
      localStorage.setItem('flort_user', JSON.stringify(adminUser));
    ...
    setLoading(false);
    if (error) return setStatus(error.message);
    if (!data) return setStatus('Kullanıcı adı veya şifre yanlış.');
    if (data.is_admin) return setStatus('Admin hesabı kullanıcı girişinden açılamaz.');

    localStorage.setItem('flort_user', JSON.stringify(data));
    setCurrentUser(data);
    setStatus('Giriş başarılı.');
  }

  function signOut() {
    localStorage.removeItem('flort_user');
    setCurrentUser(null);
  ...
  async function sendAdminReply() {
    ...
    const { error } = await supabase.from('messages').insert({
      member_username: selectedThread.member_username,
          ...
          <section className="card">
            ...
            <div className="thread-list">
              {incomingThreads.map((thread) => (
                <button
                  key={`${thread.member_username}-${thread.virtual_profile_id}`}
                  onClick={() => setSelectedThread(thread)}
                  className={selectedThread?.member_username === thread.member_username && selectedThread?.virtual_profile_id === thread.virtual_profile_id ? 'active' : ''}
                >
